...
} ## end foreach my $dir (qw(daily progress last_rotation dumps structs tables binlogs))
...
} ## end foreach my $key (qw(hostname username password))
...
sub _get_local_ip {
  ...
  foreach my $line (@out) {
    ...
    } ## end if ( $line =~ m/^\s*inet\s/)
  } ## end foreach my $line (@out)
...
} ## end sub _get_local_destination
  ...
  foreach my $db ( sort keys %{$table_ref} ) {
    ...
    foreach my $table ( sort keys %{ $table_ref->{$db} } ) {
        ...
        || $self->config()->get('MTK::MYB::DumpStruct') )
      {
        ...
        $self->logger()->log(
          message => 'Not dumping structure of Table ' . $self->dbms() . q{.} . $db . q{.} . $table . ' (' . $engine . '). DumpStruct = 0',
          level   => 'notice'
        );
        ...
        || ( $self->config()->get('MTK::MYB::CopyTable') && $engine ne 'MYISAM' ) )
      {
        ...
        $self->logger()
          ->log( message => 'Not dumping Table ' . $self->dbms() . q{.} . $db . q{.} . $table . ' (' . $engine . '). DumpTable = 0', level => 'notice' );
      }
      ...
      if ( $engine ne 'MYISAM' ) {
        ...
        $self->logger()->log(
          message => 'Storage Engine of Table '
            . $self->dbms() . q{.}
            . $db . q{.}
            . $table . ' ('
            . $engine
            . ') not MyISAM. Can not create tablefile backup.',
          level => 'debug',
        );
      } ## end if ( $engine ne 'MYISAM')
      elsif (
        $engine eq 'MYISAM'
      ...
      } ## end elsif ( $engine eq 'MYISAM'...)
      else {
        $self->status()->set_table_status( $self->dbms(), $db, $table, -5, 'tables' );    # Status -5 => do_tablefiles = 0
        $self->logger()->log(
          message => 'Not archiving tablefiles of ' . $self->dbms() . q{.} . $db . q{.} . $table . ' (' . $engine . '). CopyTable = 0',
          level   => 'notice',
        );
      } ## end else [ if ( $engine ne 'MYISAM')]
      ...
      if ( !$DBH || !$DBH->valid() ) {
        my $msg =
            'Lost mysql connection to '
          . $hostname
          . ' during backup of '
          . $self->dbms() . q{.}
          . $db . q{.}
          . $table
          . '. Backup is insconsistent due to broken locks!';
        $self->logger()->log( message => $msg, level => 'alert', );
      } ## end if ( !$DBH || !$DBH->valid...)
    }    # End of TWHILE
  } ## end foreach my $db ( sort keys ...)
...
sub _innodb_update_time {
  ...
  if ( $out && $out =~ m/^\d+$/ ) {
    $self->logger()
      ->log( message => 'Found the update time of ' . $db . q{.} . $table . ' via stat. Update time: ' . $out . ' / ' . localtime($out), level => 'debug', );
    return $out;
  }
  else {
    $out ||= '';
...
sub _binlog_archive {
  ...
  if ( $source_dir && -d $source_dir ) {
    ...
    $self->logger()->log( message => 'Source dir for ' . $self->dbms() . ' is not defined or not a directory! Not archiving binlogs.', level => 'warning', );
    return;
  }
  ...
  foreach my $file ( glob( $source_dir . '/mysql-bin.*' ) ) {
    ...
    } ## end if ( -M $file > $holdbacktime)
  } ## end foreach my $file ( glob( $source_dir...))
  ...
  } ## end foreach my $binlog_file (@out)
...
sub _can_hardlink {
  ...
  if ( $last_backup_modtime && $params->{'table_last_update'} && $params->{'table_last_update'} <= ( $last_backup_modtime - 24 * 60 ) ) {
    ...
    return;
  } ## end else [ if ( $last_backup_modtime...)]
} ## end sub _can_hardlink
...
sub _table_dump {
  ...
  } ## end foreach my $key ( qw( db table engine mysqld_host target_host...))
  ...
  if ( $last_backup_file && -e $last_backup_file && $params->{'table_last_update'} ) {
    ...
    $self->logger()->log(
      message => 'Last Backup Modtime for '
        . $params->{'db'} . q{/}
        . $params->{'table'} . ' is '
  ...
  if ( $self->_can_hardlink( $params, $last_backup_modtime, $last_backup_size ) ) {
    ...
    } ## end else [ if ( $self->dry() ) ]
  } ## end else [ if ( $self->_can_hardlink...)]
} ## end sub _table_dump
...
sub _table_copy {
  ...
  $self->logger()->log(
    message => 'Processing Tablefiles for DB: ' . $params->{'db'} . ' / Table: ' . $params->{'table'} . ' (Engine: ' . $params->{'engine'} . ')',
    level   => 'debug',
  );
  ...
  if ( !$self->sys()->run( $params->{'target_host'}, $cmd, ) ) {
    $self->logger()->log( message => 'Unable to access DB-Directory at ' . $dbdir . ' for Table ' . $params->{'table'} . '. Aborting.', level => 'warning' );
    return;
  }
...
#$cmd = '/usr/bin/find ' . $dbdir . ' -name "' . $params->{'table'} . '.MYI" -o -name "' . $params->{'table'} . '.MYD" -o -name "' . $params->{'table'} . '.frm"';
  ...
  if ($files) {
    ...
    foreach my $file ( split /\n/, $files ) {
      ...
      } ## end else [ if ( $params->{'engine'...})]
    } ## end foreach my $file ( split /\n/...)
    return $status;
  } ## end if ($files)
  else {
