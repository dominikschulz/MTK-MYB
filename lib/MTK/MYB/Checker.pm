...
} ## end foreach my $var (qw(instances dbs tables valid failed))
...
sub _check_tables {
  ...
  } ## end foreach my $db ( @{ $self->...})
  return 1;
} ## end sub _check_tables
...
sub _check_dumps {
  ...
  if ( $self->num_tables() < 1 || $pc_new < $self->min_pc() || $self->num_failed() > 0 ) {
    ...
    return 1;
  } ## end else [ if ( $self->num_tables...)]
} ## end sub _check_dumps
